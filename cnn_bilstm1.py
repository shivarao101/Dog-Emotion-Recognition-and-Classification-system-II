...
from tensorflow.keras.layers import (
    Input, Conv2D, MaxPooling2D,GlobalAveragePooling1D, BatchNormalization,
...
# CNN output shape ≈ (None, 30, 30, 128)

# -----------------------------
# Prepare for BiLSTM
# -----------------------------
# Treat one axis as time
x = layers.Reshape((x.shape[1], x.shape[2] * x.shape[3]))(x)
# Shape → (None, time_steps, features)
...
# -----------------------------
attention = layers.Attention()([x, x])  # Self-attention (Q = K = V)

# Optional residual connection (recommended)
x = layers.Add()([x, attention])

# Temporal pooling to get fixed-length vector
...
X_train, y_train = prepare_dataset(X_train_files, y_train)
X_test,  y_test  = prepare_dataset(X_test_files,  y_test)
